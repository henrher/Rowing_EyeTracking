...
path.plots <- "Plots/timediff/" # Subfolder to receive exported plots
...
i <- pupils.list$files[77] # Choose a csv file from your list to process
...
for (i in condition.trials) {
  ...
  rower.smi.bl <- baseline[, c("Pupil.Diameter.Right..mm.", "Pupil.Diameter.Left..mm.", 
                               "Gaze.Vector.Right.X", "Gaze.Vector.Right.Y", 
                               "Gaze.Vector.Left.X", "Gaze.Vector.Left.Y", 
                               "RecordingTime..ms.", "Tracking.Ratio....")]
  ...
  rower.smi.bl <- rower.smi.bl[rower.smi.bl$newtime >= crop.times.start & 
                                 rower.smi.bl$newtime <= crop.times.stop,]
  ...
  rower.smi.filtered.left["newtime_diff"] <- with(rower.smi.filtered.left, c(NA, diff(newtime)))
  rower.smi.filtered.right["newtime_diff"] <- with(rower.smi.filtered.right, c(NA, diff(newtime)))
  rower.smi.bl.left["newtime_diff"] <- with(rower.smi.bl.left, c(NA, diff(newtime)))
  rower.smi.bl.right["newtime_diff"] <- with(rower.smi.bl.right, c(NA, diff(newtime)))
  
  ## Add data loss info
  row.loss.cond.right = 9600 - nrow(rower.smi.filtered.right)
  row.loss.cond.left = 9600 - nrow(rower.smi.filtered.left)
  row.loss.bl.right = 600 - nrow(rower.smi.bl.right)
  row.loss.bl.left = 600 - nrow(rower.smi.bl.left)
  rows.in.pupil.data.cond.right = 9600 - (row.loss.cond.right) - sum(is.na(rower.smi.filtered.right$xmm_sg))
  rows.in.pupil.data.cond.left = 9600 - (row.loss.cond.left) - sum(is.na(rower.smi.filtered.left$xmm_sg))
  rows.in.pupil.data.bl.right = 600 - (row.loss.bl.right) - sum(is.na(rower.smi.bl.right$xmm_sg))
  rows.in.pupil.data.bl.left = 600 - (row.loss.bl.left) - sum(is.na(rower.smi.bl.left$xmm_sg))
  ...
  ## Add results
  to.add <- data.frame(row.loss.cond.right = 9600 - nrow(rower.smi.filtered.right),
                       row.loss.cond.left = 9600 - nrow(rower.smi.filtered.left),
                       row.loss.bl.right = 600 - nrow(rower.smi.bl.right),
                       row.loss.bl.left = 600 - nrow(rower.smi.bl.left),
                       rows.in.pupil.data.cond.right = 9600 - (row.loss.cond.right) - sum(is.na(rower.smi.filtered.right$xmm_sg)),
                       ...
                       pupil.data.percentage.bl.right = (rows.in.pupil.data.bl.right/600 *100),
                       pupil.data.percentage.bl.left = (rows.in.pupil.data.bl.left/600 *100),
                       count.NA.xmm.sg.cond.left = sum(is.na(rower.smi.filtered.left$xmm_sg)),
                       count.NA.xmm.sg.cond.right = sum(is.na(rower.smi.filtered.right$xmm_sg)),
                       count.NA.xmm.sg.bl.left = sum(is.na(rower.smi.bl.left$xmm_sg)),
                       count.NA.xmm.sg.bl.right = sum(is.na(rower.smi.bl.right$xmm_sg)),
                       count.rows.bl.right = nrow(rower.smi.bl.right),
                       count.rows.bl.left = nrow(rower.smi.bl.left),
                       count.rows.cond.right = nrow(rower.smi.filtered.right),
                       count.rows.cond.left = nrow(rower.smi.filtered.left),
                       max.timegap.bl.right = max(rower.smi.bl.right$newtime_diff, na.rm = T),
                       max.timegap.bl.left = max(rower.smi.bl.left$newtime_diff, na.rm = T),
                       max.timegap.cond.right = max(rower.smi.filtered.right$newtime_diff, na.rm = T),
                       max.timegap.cond.left = max(rower.smi.filtered.left$newtime_diff, na.rm = T),
                       min.timegap.bl.right = min(rower.smi.bl.right$newtime_diff, na.rm = T),
                       min.timegap.bl.left = min(rower.smi.bl.left$newtime_diff, na.rm = T),
                       min.timegap.cond.right = min(rower.smi.filtered.right$newtime_diff, na.rm = T),
                       min.timegap.cond.left = min(rower.smi.filtered.left$newtime_diff, na.rm = T),
                       mean.timegap.bl.right = mean(rower.smi.bl.right$newtime_diff, na.rm = T),
                       mean.timegap.bl.left = mean(rower.smi.bl.left$newtime_diff, na.rm = T),
                       mean.timegap.cond.right = mean(rower.smi.filtered.right$newtime_diff, na.rm = T),
                       mean.timegap.cond.left = mean(rower.smi.filtered.left$newtime_diff, na.rm = T),
                       percent.NA.sg.cond.left = sum(is.na(rower.smi.filtered.left$xmm_sg)) / nrow(rower.smi.filtered.left[rower.smi.filtered.left$xmm_sg, ]),
                       percent.NA.sg.cond.right = sum(is.na(rower.smi.filtered.right$xmm_sg)) / nrow(rower.smi.filtered.right[rower.smi.filtered.right$xmm_sg, ]),
                       percent.NA.sg.bl.left = sum(is.na(rower.smi.bl.left$xmm_sg)) / nrow(rower.smi.bl.left[rower.smi.bl.left$xmm_sg, ]),
                       percent.NA.sg.bl.right = sum(is.na(rower.smi.bl.right$xmm_sg)) / nrow(rower.smi.bl.right[rower.smi.bl.right$xmm_sg, ]),
                       ...
                       sd.diameter.sg.bl.left = sd(rower.smi.bl.left$xmm_sg, na.rm = T),
                       sd.diameter.sg.bl.right = sd(rower.smi.bl.right$xmm_sg, na.rm = T),
                       ...
                       sd.diameter.sg.cond.left = sd(rower.smi.filtered.left$xmm_sg, na.rm = T),
                       sd.diameter.sg.cond.right = sd(rower.smi.filtered.right$xmm_sg, na.rm = T),
...
write.table(results, file='C:/Users/henrher/OneDrive - Universitetet i Oslo/Documents/R and analysis/Pupil and Analysis/Results and Datasets 2022//results_SMI_newdata_after_revision.txt')
